package com.lorin.hadoop.pageRank;
...
public class PageRankJob {
    ...
    private static Map<String, String> peoplerank() {
        ...
        path.put("input", HDFS + "/usr/hdfs/pagerank");// HDFS的目录
        path.put("input_pr", HDFS + "/usr/hdfs/pagerank/pr");// pr存储目
        path.put("tmp1", HDFS + "/usr/hdfs/pagerank/tmp1");// 临时目录,存放邻接矩阵
        path.put("tmp2", HDFS + "/usr/hdfs/pagerank/tmp2");// 临时目录,计算到得PR,覆盖input_pr

        path.put("result", HDFS + "/usr/hdfs/pagerank/result");// 计算结果的PR
        return path;
